...
func transit(current_state, params={}, local_params={}):
	...
	if is_nested:
		var is_nested_exit = nested_states[nested_states.size()-1] == YAFSM_State.EXIT_STATE
		if is_nested_exit:
		...
		for transition in from_transitions_array:
			...
			if next_state:
				if "states" in end_state_machine.states[next_state]:
					...
					next_state = join_path(base_path, [next_state, YAFSM_State.ENTRY_STATE])
				else:
					# Construct next state into absolute path
...
func get_entries():
	return _transitions[YAFSM_State.ENTRY_STATE].values()
	
func get_exits():
	return _transitions[YAFSM_State.EXIT_STATE].values()

func has_entry():
	return YAFSM_State.ENTRY_STATE in _states

func has_exit():
	return YAFSM_State.EXIT_STATE in _states
