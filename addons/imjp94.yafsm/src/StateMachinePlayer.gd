...
func _on_state_changed(from, to):
	match to:
		YAFSM_State.ENTRY_STATE:
			emit_signal("entered", "")
		YAFSM_State.EXIT_STATE:
			set_active(false) # Disable on exit
			emit_signal("exited", "")
	
	if to.ends_with(YAFSM_State.ENTRY_STATE) and to.length() > YAFSM_State.ENTRY_STATE.length():
		# Nexted Entry state
		var state = path_backward(get_current())
		emit_signal("entered", state)
	elif to.ends_with(YAFSM_State.EXIT_STATE) and to.length() > YAFSM_State.EXIT_STATE.length():
...
func is_entered():
	return YAFSM_State.ENTRY_STATE in stack

# Return if player ended
func is_exited():
	return get_current() == YAFSM_State.EXIT_STATE
