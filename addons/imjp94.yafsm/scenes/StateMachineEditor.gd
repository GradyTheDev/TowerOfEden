...
const YAFSM_State = preload("../src/states/State.gd")
	...
	if _last_index > index:
		...
		if layer:
			...
			if node:
				...
				var cond_2 = (nested_layer.content_nodes.get_node_or_null(NodePath(YAFSM_State.ENTRY_STATE)) == null)  # there is no entry state in the node
				var cond_3 = (nested_layer.content_nodes.get_node_or_null(NodePath(YAFSM_State.EXIT_STATE)) == null)  # there is no exit state in the node
...
func _on_context_menu_index_pressed(index):
	...
	match index:
		...
		1: # Add Entry
			if YAFSM_State.ENTRY_STATE in current_layer.state_machine.states:
				push_warning("Entry node already exist")
				return
			new_node.name = YAFSM_State.ENTRY_STATE
		2: # Add Exit
			if YAFSM_State.EXIT_STATE in current_layer.state_machine.states:
				push_warning("Exit node already exist")
				return
			new_node.name = YAFSM_State.EXIT_STATE
...
func _request_connect_from(layer, from):
	if from == YAFSM_State.EXIT_STATE:
		return false
	return true

func _request_connect_to(layer, to):
	if to == YAFSM_State.ENTRY_STATE:
		return false
	return true
